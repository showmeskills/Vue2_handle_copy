...
//深层次嵌套会递归 递归多了性能差 不存在属性监控不到 存在的属性要重写 vue3 - proxy
...
export function defineReactive(target,key,value){//闭包 属性劫持
    let childOb = observe(value); // 对所有的对象都进行属性劫持; 深度属性劫持
    let dep = new Dep(); //每一个属性都有一个Dep 而且是闭包不会背销毁的
    Object.defineProperty(target,key,{
        get(){//取值的时候 会执行get
            //console.log("用户取值 key",key)
            if(Dep.target){ // 做判断的原因是，当前html中的数据是否在页面中使用(不使用 依赖收集)
                dep.depend();//让这个属性的手机记住当前的
                if(childOb){
                    childOb.dep.depend();//让数组和对象本身也是实体依赖收集
