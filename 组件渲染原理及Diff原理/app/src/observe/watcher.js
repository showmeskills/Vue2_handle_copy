...
class Watcher { //不同的组件有不同的watcher 目前只有一个 渲染根实例
    constructor(vm, exprOrFn, options, cb) {
        ...
        if (typeof exprOrFn === "string") {
            ...
            this.getter = exprOrFn; //getter 调用这个函数可以发生取值操作
        }
    ...
    evaluate() {
        this.value = this.get(); //获取用户的绑定值
        this.dirty = false;
    }
    get() {
        //Dep.target = this;//静态属性就是只有一份 在这个给组件赋值;this 就是渲染谁就是指谁
        pushTarget(this);
        let value = this.getter.call(this.vm); //会去vm上取值 vm._update(vm._render()) 取挂在的变量
    ...
    run() {
        ...
        if (this.user) {
            this.cb(this.vm,newValue, oldValue)
        }
    }
}
let queue = [];
